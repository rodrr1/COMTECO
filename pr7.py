...
#broker = '52.67.47.167'
#broker = 'localhost'
broker = 'monitoreroiotbeymar.online'
port = 1883
topic = "testtopic/1"
#PRUEBAAAAA---------------------------------
topic2 = "testtopic/2"
...
def subscribe(client: mqtt_client):
    ...
    def on_message(client, userdata, msg):
        global last_update_time  # Asegúrate de que last_update_time sea global
        # Actualiza last_update_time con la hora y fecha actual
        ...
        if last_row_index is not None:
            tree.item(last_row_index, values=[
                last_update_time,  # Hora y fecha del último mensaje
                "ESMERALDA", "Activa", "Descripción 1", "Recurso 1", "Condición 1"
            ])
        # Si no hay última fila, inserta una nueva fila con la hora y fecha actualizada
        else:
            last_row_index = tree.insert("", "end", values=[
                last_update_time,  # Hora y fecha del último mensaje
                "ESMERALDA", "Activa", "Descripción 1", "Recurso 1", "Condición 1"
            ])
...
data = [
    [time.strftime('%Y-%m-%d %H:%M:%S'), "ESMERALDA", "Desactivada", "Descripción 2", "Recurso 2", "Condición"],
...
if __name__ == "__main__":
    ...
    while True:
        ...
        client.loop_stop()
        # Supongamos que recibes la hora y fecha actual del sistema al recibir un mensaje:
        #last_update_time = time.strftime('%Y-%m-%d %H:%M:%S')